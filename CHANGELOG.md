...
## Round 9 — editor workflow unification & contribution approval fixes (landed)

- **Editors submit through the normal queue**: removed the editor direct-add-to-catalog shortcut from `Contribute.tsx` (Value/Comment fields, validation, payload, and editor-specific header/blurb). The backend direct-add path was already superuser-only, so editors now flow through pending → review exactly like contributors.
- **Editors can approve their own submissions**: dropped the "peer review required" block in `api/v1/views.py` `approve()` and the matching frontend notice in `ContributionDetail.tsx`. Region-assignment is still enforced; editors just aren't blocked from reviewing their own entries.
- **Review UI**: on `ContributionDetail.tsx`, removed the Value field and made Comment optional for Approve but required for Reject / Request Revision, with inline `border-destructive` highlight and error text matching the rest of the form.
- **Resubmit 500 (v2)**: `api/v2/views.py` had a function-scope local `from common.models import Contribution`, which shadowed the module-level import and raised `UnboundLocalError` on the resubmit branch that ran first. Removed the redundant local import.
- **Approve 500 after resubmit**: `contribution_apply._create_postmark_in_catalog` / `_update_postmark_in_catalog` were looking up `Region.administrative_unit`, a field that no longer exists (migration `0043` moved that FK onto `UserLocationAssignment`). Now resolved by case-insensitive `name` / `abbrev` match against existing Regions; fails loudly if no match instead of creating phantom rows. Added `logger.exception` on both paths so the bare `except` stops swallowing tracebacks.
- **Admin CSV import for timestamped models**: exports emit ISO 8601 (`2026-04-20T15:30:00.123456+00:00`) but django-import-export's default `DateTimeWidget` doesn't accept that shape on import, so round-tripping any `TimestampedModel` resource (Region included) failed with "could not be parsed using defined formats." Added an `IsoDateTimeWidget` (uses `django.utils.dateparse.parse_datetime`) and wired it for `created_date` / `modified_date` on `TimestampedModelResource`.
