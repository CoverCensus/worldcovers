# VPHC Ingestion Decisions

These decisions were made during ingestion of the Virginia Postal History
Catalog (VPHC). They cover town matching, public descriptions, review
information, postmaster service dates, and preserving existing data during
Marking edits. The town matching and Marking edit rules also apply outside
VPHC ingestion. The postmaster source data discussed here comes from VPHC.

The entries retain their original dates, evidence, and follow-up notes.
Counts, test results, and pending work describe the state at those dates.
See [DECISIONS.md](../../DECISIONS.md) for other application and operations
decisions. Code paths in these entries are relative to the repository root.
...
Important: **This will look like an obvious missing feature to the next person reading the model. It is
...
Warning: **Not swept: markings already approved with notation in `desc`.** Four exist in the local
...
- `lettering` is present on the 310 edit submissions and is `null` on 305 of them
  (`"Serif"` x3, `"Outline"` x2) -- `toStr(null)` -> `""`, so the renderer prints "-".
