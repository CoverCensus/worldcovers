...
Scope (Phase 2, first cut):
  * Marking submissions only (type in TOWNMARK/RATEMARK/AUXMARK).
  * Cover submissions (submission_kind == "cover" or type in FC/FL)
    raise ContributionApplyNotImplemented.
  * Edit flow (edit_postmark_id present in payload) is ignored: a new
    Marking is always created. Approving an edit-draft today therefore
    produces a duplicate row. Follow-up.
  * No DateSeen rows are created here. Dates-seen are added later by
    editors via direct catalog import.

The function reads contrib.submitted_data (a JSON blob written by the
v2 ContributionSubmitView) and creates one Marking + one-or-more Image
rows + zero-or-more Citation rows, all under the caller's transaction.
Returns the created Marking.
"""
...
def apply_contribution_to_catalog(contrib):
    ...
    if submission_kind == "cover" or sub_type in _COVER_TYPES:
        raise ContributionApplyNotImplemented(
            "Cover-submission approval is not yet implemented; only "
            "TOWNMARK/RATEMARK/AUXMARK submissions can be approved."
        )
