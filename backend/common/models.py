...
class CommentSubmission(models.Model):
    """
    Moderation ticket for user-submitted comments on records/collections.
    Comments are only public after editor approval.
    """

    TARGET_POSTMARK = "postmark"
    TARGET_COLLECTION = "collection"
    TARGET_CHOICES = [
        (TARGET_POSTMARK, "Record"),
        (TARGET_COLLECTION, "Collection"),
    ]
    ...
    target_type = models.CharField(max_length=20, choices=TARGET_CHOICES, default=TARGET_POSTMARK)
    ...
    review_reason = models.TextField(
        blank=True,
        help_text="Editor feedback; required when denied.",
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["postmark", "status"]),
