...
-- 7. Drop the existing overly-permissive policy on login_requests
DROP POLICY IF EXISTS "Authenticated users can view login requests" ON public.login_requests;

-- 8. Create new restrictive policy: only admins can view login requests
CREATE POLICY "Only admins can view login requests"
