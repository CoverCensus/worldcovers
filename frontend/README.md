...
From the project root: `woco dev`. With `DEBUG=True` (the default) this starts both the Vite dev server (HMR, on :8080) and Django (on :8000) in the same terminal; Ctrl+C kills both. Open **http://localhost:8080/**.
...
Set `DEBUG=False` in `.env` and run `woco dev`. It builds `frontend/dist/` and starts Django on **http://127.0.0.1:8000/**. Use this to sanity-check the production bundle before pushing.
